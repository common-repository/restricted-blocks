...
class Daextrebl_Pagination {
	...
	/**
	 * Set the the current page parameter by getting it from $_GET['p'], if it's not set or it's not > than 0 then set
	 * it to 1.
	 */
	...
	private function get_pagenum_link( $id ) {
		...
		if ( isset( $_GET['s'] ) and strlen( trim( $_GET['s'] ) ) > 0 ) {
			$filter = '&s=' . sanitize_text_field( $_GET['s'] );
		} else {
			$filter = '';
		}
